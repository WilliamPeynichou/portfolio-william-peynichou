...
function Opener() {
  ...
  const handleScroll = useCallback(() => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      const phase1Progress = Math.min(scrollY / windowHeight, 1)
      // Phase 2 : Deuxième scroll (1 à 2 fenêtres) - Nom disparaît
      const phase2Progress = Math.max(0, Math.min((scrollY - windowHeight) / windowHeight, 1))

      if (scrollY < windowHeight) {
        setNameOpacity(phase1Progress)
        setBackgroundZoom(1 + (phase1Progress * 4))
        setShowBackground(true)
        setShowArrow(phase1Progress < 1)
        ...
        // Background n'est plus visible - le démonter pour libérer le GPU
        setNameOpacity(0)
        setBackgroundZoom(5)
  ...
  return (
    <>
      <div className="fixed top-0 left-0 w-full h-full z-0 overflow-hidden bg-black">
        {/* Background avec zoom - démonté quand invisible pour libérer le GPU */}
        <Background zoom={backgroundZoom} visible={showBackground} />
