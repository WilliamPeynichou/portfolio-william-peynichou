  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center z-10" style={{ opacity: titleOpacity }}>
      ...
      <p className="text-xl md:text-2xl text-white/70 font-light mt-4">
        AI Pipeline / Developer Tool
      </p>
    </div>
  )
}

function AiCodePipeline() {
  const { language } = useLanguage()
  const navigate = useNavigate()
  const [scrollProgress, setScrollProgress] = useState(0)
  const [titleOpacity, setTitleOpacity] = useState(1)

  const workflow = workflows.find(w => w.slug === 'ai-code-pipeline')
  ...
  return (
    <div className="relative font-sans text-white bg-black">
      ...
      <div className="fixed top-0 left-0 w-full h-full z-0 overflow-hidden bg-black">
        <TitleOpener title="AI CODE PIPELINE" titleOpacity={titleOpacity} />
        {scrollProgress < 0.5 && (
      ...
      <div className="relative z-10 mt-[100vh]">
        <div className="min-h-screen rounded-t-[3rem] bg-black pt-20 pb-20">
          <div className="max-w-7xl mx-auto px-4 md:px-12">
            ...
            <div className="mb-16">
              <h1 className="text-4xl md:text-8xl font-light tracking-tight mb-6">{workflow.title}</h1>
              <p className="text-xl md:text-2xl text-gray-400 font-light max-w-2xl">{workflow.type}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-12 mb-24">
              <div className="md:col-span-4 space-y-8">
                ...
                <div>
                  <h3 className="text-sm font-mono text-gray-500 uppercase tracking-widest mb-2">GitHub</h3>
                  <a
                    href={workflow.githubUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-white/70 hover:text-white transition-colors text-sm font-mono underline underline-offset-4"
                  >
